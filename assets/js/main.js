...
(function initParticles() {
  ...
  function draw() {
    ...
    particles.forEach(p => {
      ...
      ctx.fillStyle = `rgba(1, 61, 237, ${opacity})`;
      ctx.fill();
      ...
      particles.forEach(q => {
        ...
        if (dist < 120) {
          ...
          ctx.moveTo(p.x, p.y);
          ctx.lineTo(q.x, q.y);
          ctx.strokeStyle = `rgba(1, 61, 237, ${opacity * 0.25 * (1 - dist / 120)})`;
          ctx.lineWidth = 0.5;
          ctx.stroke();
        }
      });
  ...
  function setBtn(state) {
    const states = {
      ...
      loading: {
        ...
        bg: 'rgba(255,255,255,0.7)', color: '#0133c4', disabled: true,
      },
      success: {
        html: `<svg xmlns="http://www.w3.org/2000/svg" width="18" height="18" viewBox="0 0 24 24" fill="none"
                 stroke="currentColor" stroke-width="2.5" stroke-linecap="round" stroke-linejoin="round">
...
/* ─────────────────────────────────────────────────────────
   11. EXTRA SCRIPTS BUNDLE (Combined Legacy functions)
   ───────────────────────────────────────────────────────── */
...
document.addEventListener('DOMContentLoaded', () => {
  ...
  // Mobile navigation compatibility layer (mobileMenuBtn support)
  ...
  // Floating Socials Action Button (FAB)
  ...
  // Decrypt obfuscated contact information dynamically
  document.querySelectorAll('[data-obf-href]').forEach(el => {
    el.setAttribute('href', atob(el.getAttribute('data-obf-href')));
  });
  document.querySelectorAll('[data-obf-text]').forEach(el => {
    el.textContent = atob(el.getAttribute('data-obf-text'));
  });
  document.querySelectorAll('[data-obf-title]').forEach(el => {
    el.setAttribute('title', atob(el.getAttribute('data-obf-title')));
  });
});
